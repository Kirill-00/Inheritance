...
void main()
{
	...
	char number[10] = {};
	char src_filename[_MAX_FNAME] = {}; //_MAX_FNAME - встроенная константа, которавя содержит максимально возможную длинну имени файла
	char wal_filename[_MAX_FNAME] = {};
	char dhcpd_filename[_MAX_FNAME] = {};
	//src (source) - фаил источник из которого изначально берутся MAC и IP адреса
	//dst (destination) - конечный файл в котором столбики будут поменяны местами
	...
	//strcat(dst, src); //содержимое строки src добовляет в конец строки dst
	//strcat - выполняет конкатенацию строк,
	//а именно к первой строке(filename) добовляет вторую строку (" RAW.txt");
	...
	//strcpy() копирует строку stc в строку dst
}