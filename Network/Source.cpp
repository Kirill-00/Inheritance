...
void main()
{
	...
	char number[10] = {};
	char src_filename[_MAX_FNAME] = {}; //_MAX_FNAME - встроенна€ константа, которав€ содержит максимально возможную длинну имени файла
	char wal_filename[_MAX_FNAME] = {};
	char dhcpd_filename[_MAX_FNAME] = {};
	//src (source) - фаил источник из которого изначально берутс€ MAC и IP адреса
	//dst (destination) - конечный файл в котором столбики будут помен€ны местами
	...
	//strcat(dst, src); //содержимое строки src добовл€ет в конец строки dst
	//strcat - выполн€ет конкатенацию строк,
	//а именно к первой строке(filename) добовл€ет вторую строку (" RAW.txt");
